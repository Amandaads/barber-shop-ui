...
export class ClientTableComponent implements AfterViewInit, OnChanges , OnDestroy {
  ...
  constructor(
    @Inject(SERVICES_TOKEN.DIALOG) private  dialogManagerService: IDialogManagerService, 
   ) {}
  ...
  delete(client: ClientModelTable){
    this.dialogManagerService.showYesNoDialog(
      YesNoDialogComponent, {title: 'Exclusao de cliente', content: `Confirma a exclusao do cliente ${client.name}`}
    )
    .subscribe(result => {
      if (result){
